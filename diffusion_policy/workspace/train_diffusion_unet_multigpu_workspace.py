...
class OriginalWorkspaceCheckpointCallback(pl.Callback):
    ...
    def _run_evaluation_tasks(self, trainer, pl_module, cfg, current_epoch):
        ...
        if self._should_run_rollout(current_epoch, cfg):
            step_log.update(
                self._run_environment_rollout(pl_module, policy, current_epoch)
            )

        # Run diffusion sampling
        if self._should_run_sampling(current_epoch, cfg):
            step_log.update(
                self._run_diffusion_sampling(pl_module, policy, current_epoch)
            )
    ...
    def _get_evaluation_policy(self, pl_module, cfg):
        ...
        if (
            cfg.training.use_ema
            and hasattr(pl_module, "ema_model")
            and getattr(pl_module, "ema_model", None) is not None
        ):
    ...
    def _run_environment_rollout(
        self, pl_module, policy, current_epoch, current_step=None
    ):
    ...
    def _run_diffusion_sampling(self, pl_module, policy, current_epoch):
        ...
        if train_sampling_batch is not None:
            ...
            with torch.no_grad():
                ...
                obs_dict = dict_apply(
                    train_sampling_batch["obs"], lambda x: x.to(pl_module.device)
                )
                ...
                pl_module.log(
                    "train_action_mse_error", mse, on_epoch=True, sync_dist=True
                )
            ...
            print(
                "Warning: train_sampling_batch is not set, skipping sampling evaluation."
            )
    ...
    def _handle_checkpoint_saving_only(self, trainer, cfg, current_epoch):
        ...
        epoch_ckpt_path = os.path.join(
            self.workspace.output_dir, "checkpoints", f"epoch_{current_epoch:04d}.ckpt"
        )
    ...
    def _try_fallback_metric(self, metric_dict):
        ...
        if "val_loss" in metric_dict and self.topk_manager is not None:
            ...
            try:
                ...
                if original_key is not None and hasattr(
                    self.topk_manager, "monitor_key"
                ):
...
class DiffusionPolicyMultiGpuLightningModule(pl.LightningModule):
    ...
    def _setup_env_runner(self):
        ...
        self.env_runner = hydra.utils.instantiate(
            self.cfg.task.env_runner, output_dir=self.output_dir
        )
    ...
    def configure_optimizers(self):
        ...
        ), "Model should be configured before optimizer setup"
        ...
        try:
            ...
            print(
                "Warning: `trainer.estimated_stepping_batches` not available. Falling back to manual calculation."
            )
    ...
    def _log_training_metrics(self, raw_loss, batch_idx):
        ...
        if (
            ...
            and hasattr(self, "json_logger")
            and self.json_logger is not None
        ):
            step_log = {
    ...
    def _is_last_batch_in_epoch(self, batch_idx):
        ...
        try:
            ...
            raise ValueError(
                "Please set `num_training_batches` in the config if the dataloader does not support __len__."
            )
...
class DiffusionMultiGpuDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        full_dataset = hydra.utils.instantiate(self.cfg.task.dataset, **mask_params)
        ...
        if stage == "fit":
            ...
            if self._is_main_process():
                if os.path.exists(val_mask_path):
                    ...
                    print(
                        f"Main process loading existing validation mask: {val_mask_path}"
                    )
                else:
    ...
    def _wait_for_validation_mask(self, val_mask_path, timeout=300):
        ...
        while not os.path.exists(val_mask_path):
            if time.time() - start_time > timeout:
                raise TimeoutError(
                    f"Timeout waiting for validation mask: {val_mask_path}"
                )
    ...
    def _setup_for_training(self, full_dataset):
        ...
        self.normalizer = self.train_dataset.get_normalizer(
            **self.cfg.task.dataset.get("normalizer", {})
        )
    ...
    def train_dataloader(self):
        ...
        if self.train_dataset is None:
            raise RuntimeError(
                "No training dataset available. Call setup('fit') first."
            )
    ...
    def val_dataloader(self):
        ...
        if self.val_dataset is None:
            raise RuntimeError(
                "No validation dataset available. Call setup('fit') first."
            )
...
class TrainDiffusionUnetMultiGpuWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.lightning_model = DiffusionPolicyMultiGpuLightningModule(
    ...
    def run(self):
        ...
        checkpoint_callback = OriginalWorkspaceCheckpointCallback(
            self, enable_validation_during_training=False
        )
        ...
        # start training
        trainer.fit(model=lightning_model, datamodule=data_module, ckpt_path=ckpt_path)
