...
class RandomPolicy(BaseImagePolicy):
    """Random policy for testing that follows BaseImagePolicy interface"""
    
    def __init__(self, action_space, shape_meta=None, device='cuda', dtype=torch.float32):
        """
        Initialize RandomPolicy
        
        Args:
            action_space: Gym action space
            shape_meta: Shape metadata (compatible with BaseImagePolicy interface)
        ...
        # Create a dummy parameter to establish device and dtype
        # This follows the ModuleAttrMixin pattern
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            obs_dict: Dictionary of observations with shape B,To,*
            
        Returns:
            Dictionary containing action tensor with shape B,Ta,Da
        """
        # Get batch size from any observation
        batch_size = list(obs_dict.values())[0].shape[0]
        
        # Sample random actions for each item in batch
        actions = []
        for _ in range(batch_size):
            actions.append(self.action_space.sample())
        
        # Convert to tensor and ensure correct types
        ...
        # Add action sequence dimension if needed (Ta=1 for single-step actions)
        if len(action_tensor.shape) == 2:  # B, Da
    ...
    def set_normalizer(self, normalizer: LinearNormalizer):
        """Set normalizer (not used for random policy but required by interface)"""
        self.normalizer = normalizer
        
    def forward(self, *args, **kwargs):
        """Forward pass (not used for this policy but may be required by some frameworks)"""
        pass


class PickCubeImageRunner(BaseImageRunner):
    """UR10e + Robotiq PickCube image runner using ManiSkill"""
    ...
    def run(self, policy: BaseImagePolicy):
        ...
        for i in range(n_inits):
            ...
            try:
                ...
                agent.reset(UR10eRobotiq.keyframes["rest"].qpos)
                agent.robot.set_pose(UR10eRobotiq.keyframes["rest"].pose)
                policy.reset()
