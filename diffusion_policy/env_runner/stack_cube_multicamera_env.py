...
@register_env("StackCube-MultiCamera", max_episode_steps=50)
class StackCubeMultiCameraEnv(BaseEnv):
    """
    **Task Description:**
    ...
    **Success Conditions:**
    ...
    def __init__(
        self, *args, robot_uids="panda_wristcam", robot_init_qpos_noise=0.02, num_envs=1, reconfiguration_freq=None, **kwargs
    ):
    ...
    def _default_sensor_configs(self):
        ...
        # return CameraConfig("render_camera", render_pose, 512, 512, 1, 0.01, 100)
        return [
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        radius = torch.linalg.norm(torch.tensor([0.02, 0.02], device=self.device)) + 0.001
        ...
        # Temporarily disable randomizing colors to simplify the task
        # for cube in [self.cubeA, self.cubeB]:
        #     for i, obj in enumerate(cube._objs):
        #         random_colors = self._batched_episode_rng[i].uniform(low=0.0, high=1.0, size=(3,)).tolist() + [1] # [R, G, B, A]
        ...
        #                     part.material.set_base_color(random_colors)
