...
def evaluate_single_checkpoint(workspace, data_module, ckpt_path, wandb_logger, use_wandb=True):
    """
    ...
    try:
        ...
        lightning_model.load_state_dict(checkpoint['state_dicts']['lightning_model'], strict=False)
        ...
        if hasattr(trainer, 'logged_metrics') and 'val_loss' in trainer.logged_metrics:
            val_loss = trainer.logged_metrics['val_loss'].item() if torch.is_tensor(trainer.logged_metrics['val_loss']) else trainer.logged_metrics['val_loss']
    ...
    if results:
        ...
        if use_wandb and wandb_logger and hasattr(wandb_logger, 'experiment'):
            ...
            for col in df.columns:
                if col not in ['checkpoint', 'checkpoint_path'] and pd.api.types.is_numeric_dtype(df[col]):
            ...
            for col in numeric_cols:
                if df[col].notna().any():
                    ...
                    if any(keyword in col.lower() for keyword in ['loss', 'error', 'mse']):
                    ...
                    summary_data.update({
                        ...
                        f'eval_best/{col}_checkpoint': df.loc[best_idx, 'checkpoint'],
                        f'eval_best/{col}_epoch': df.loc[best_idx, 'epoch'] if 'epoch' in df.columns else -1,
                    })
            ...
            import wandb
            table_columns = ['checkpoint', 'epoch'] + [col for col in numeric_cols if col != 'epoch']
            table_data = []
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate all checkpoints using Lightning workspace")
    parser.add_argument("--config", required=True, help="Path to config file")
    parser.add_argument("--checkpoint_dir", required=True, help="Directory containing training outputs")
